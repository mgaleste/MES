...
if (((!empty($sid)) && $task == 'edit') || ((!empty($sid)) && $task == 'view')) {
        $retrieveProcess = $recon->retrieveCustomQuery("SELECT processId, processName, processCode, isManual, isMachine, isInspected, isSubcon FROM $userTable m WHERE m.processId='$sid'");

        $processData = explode("|",$retrieveProcess[0]);
        $processId = $processData[0];
        $processName = $processData[1];
        $processCode = $processData[2];
        $isManual = $processData[3];
        $isMachine = $processData[4];
        $isInspected = $processData[5];
        $isSubcon = $processData[6];
		...
		if (empty($error_messages)) {
			//Save Posted Array Values to Database
                        ...
                        if (empty($queryResult)) {
                                        ...
                                        $activity 	=	"Update Process : $caption";
...
if (isset($_GET['search'])) {
   ...
   $paginate_arr['addquery'] 	= " WHERE processName LIKE '%$psearch%' ";
}

$paginate_arr['paginatequery'] 	= "SELECT processId, processName, isManual, isMachine, isInspected, isSubcon FROM $userTable";
    ...
    if (count($_POST['hidden_selected']) > 0) {
		...
		foreach($delArr as $did){
			$taskCBox		= 	"delete";
                        ...
                        $retrieveProcess = $recon->retrieveCustomQuery("select concat(processName,' | ',processCode) from processs where processsId='$did'");
...
if ($task == 'delete') {
	...
	$activity 	=	"$task Service Provider process ".$retrieveProcess[0];
	$fieldArray = array ("statusId"=>"-1");
