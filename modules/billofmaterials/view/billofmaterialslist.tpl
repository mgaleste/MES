			...
			<table width="100%" cellpadding="0" cellspacing="0" id="myTable" class="tablesorter">					
				<thead>		
					...
					<tr valign="middle">
						...
						<th width="20%" class="table_line1_left">Bill of Material Id</th>
			...
			<?php
				...
				if($num_rows>0){
					...
					for($i=0;$i<$num_rows;$i++){
						...
						$ids 				= mysql_result($result, $i, "billofmaterialsId");
						
						$itemId		 	= mysql_result($result, $i, "itemId");
                                                
                                                $revision		 	= mysql_result($result, $i, "revision");

                                                $itemName                    = $recon->retrieveCustomQuery("select itemName from item where itemId='$itemId'");
                                                $itemCode                    = $recon->retrieveCustomQuery("select itemCode from item where itemId='$itemId'");
						...
						$link				= "index.php?mod=$mod&type=$type&task=view&sid=$ids";
							...
							echo '	<td align="center" class="table_line2_"><input class="ibox" value="'.$ids.'" name="delAnn[]" onclick="checkAllFields(2);" type="checkbox"></td>
									<td class="table_line2_left" id="'.$link.'" onClick="viewRecord(event,this.id);">'.$ids.'&nbsp;</td>
									<td class="table_line2_left">'.stripslashes(html_entity_decode($itemName[0])).'&nbsp;</td>
                                                                        <td class="table_line2_left">'.stripslashes(html_entity_decode($itemCode[0])).'&nbsp;</td>
