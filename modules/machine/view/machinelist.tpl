			...
			<table width="100%" cellpadding="0" cellspacing="0" id="myTable" class="tablesorter">					
				<thead>		
					...
					<tr valign="middle">
						...
						<th width="20%" class="table_line1_left">Machine Name</th>
						<th width="20%" class="table_line1_left">Machine Code</th>
                                                <th width="20%" class="table_line1_left_right">Process</th>
					</tr>
					<tr valign="middle"><th class="bar" colspan="4"></th></tr>
				</thead>
			...
			<?php
				...
				if($num_rows>0){
					...
					for($i=0;$i<$num_rows;$i++){
						...
						$ids 				= mysql_result($result, $i, "machineId");
						$machineName		 	= mysql_result($result, $i, "machineName");
						$machineCode		 	= mysql_result($result, $i, "machineCode");
                                                $processId		 	= mysql_result($result, $i, "processId");
						
                                                $processName                    = $recon->GetValue('processName','process','processId="'.$processId.'" ');
						...
						$link				= "index.php?mod=$mod&type=$type&task=view&sid=$ids";
							...
							echo '	<td align="center" class="table_line2_"><input class="ibox" value="'.$ids.'" name="delAnn[]" onclick="checkAllFields(2);" type="checkbox"></td>
									<td class="table_line2_left" id="'.$link.'" onClick="viewRecord(event,this.id);">'.$machineName.'&nbsp;</td>
									<td class="table_line2_left">'.stripslashes(html_entity_decode($machineCode)).'&nbsp;</td>
                                                                        <td class="table_line2_left_right">'.stripslashes(html_entity_decode($processName)).'&nbsp;</td>';
