...
if (isset($_POST['hidden_selected'])) {
    if (count($_POST['hidden_selected']) > 0) {
		...
		foreach($delArr as $did){
			$cBoxtask		= 	"delete";
			$title 		=	$core->getIdCaption("username",$userTable,"username='$did'");
			$caption 	= 	(empty($title)) ? $type : $title;
			$activity 	=	"$task $caption";
                        $users->deleteRecord($userTable, "username='$did'");
			$core->logAdminTask($admin,$type,$cBoxtask,$activity);
		}
...
if ($task == 'delete') {
    ...
    $users->deleteRecord($userTable, "username='$sid'");
    header("location:index.php?mod=$mod&type=$type");
}
