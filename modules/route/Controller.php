...
//variables to be used
	...
	$mod_dir		=	$controller_var->retrieve_paths('route','modular');
...
$type 					= (!empty($_GET['type'])) ? $_GET['type'] : "";
 ...
 if(!empty($task) && $task != 'changestatus'){
	include_once $dir . $type.'Model.php';
	include_once $viewdir . $type.'.tpl';
 }else if(!empty($task) && $task == 'changestatus'){
    include_once $dir . $type.'Model.php';
    include_once $viewdir . $type.$task.'.tpl';
 }else{
	include_once $dir . $type.'Model.php';
