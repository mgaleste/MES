			...
			<table width="100%" cellpadding="0" cellspacing="0" id="myTable" class="tablesorter">					
				<thead>		
					...
					<tr valign="middle">
						...
						<th width="20%" class="table_line1_left_right">Item Type</th>
					</tr>
			...
			<?php
				...
				if($num_rows>0){
					...
					for($i=0;$i<$num_rows;$i++){
						...
						$ids 				= mysql_result($result, $i, "itemId");
						$itemName		 	= mysql_result($result, $i, "itemName");
						$itemCode		 	= mysql_result($result, $i, "itemCode");
						$itemTypeId		 	= mysql_result($result, $i, "itemTypeId");
                                                
                                                $itemType                    = $recon->GetValue('itemTypeName','itemtype','itemTypeId="'.$itemTypeId.'"');
						...
						$link				= "index.php?mod=$mod&type=$type&task=view&sid=$ids";
							...
							echo '	<td align="center" class="table_line2_"><input class="ibox" value="'.$ids.'" name="delAnn[]" onclick="checkAllFields(2);" type="checkbox"></td>
									...
									<td class="table_line2_left">'.stripslashes(html_entity_decode($itemCode)).'&nbsp;</td>
                                                                        <td class="table_line2_left">'.stripslashes(html_entity_decode($itemType)).'&nbsp;</td>';
