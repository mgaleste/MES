			...
			<table width="100%" cellpadding="0" cellspacing="0" id="myTable" class="tablesorter">
				<thead>
					...
					<tr valign="middle">
						...
						<th width="20%" class="table_line1_left">Process Name</th>
						<th width="20%" class="table_line1_left">Process Code</th>
						<th width="10%" class="table_line1_left">Manual</th>
                                                <th width="10%" class="table_line1_left">Machine</th>
                                                <th width="10%" class="table_line1_left">Inspected</th>
                                                <th width="10%" class="table_line1_left">Subcontracted</th>
                                                <th width="10%" class="table_line1_left">Multiple Output</th>
					</tr>
					<tr valign="middle"><th class="bar" colspan="8"></th></tr>
				</thead>

				<tfoot>
					<tr valign="middle"><td class="bar" colspan="8"></td></tr>
					<tr valign="middle">
						...
						<td width="20%" class="table_line1_left"></td>
                                                ...
                                                <td width="10%" class="table_line1_left"></td>
                                                <td width="10%" class="table_line1_left"></td>
						<td width="15%" class="table_line1_left_right"></td>
					</tr>
					<tr valign="middle">
						<td class="table_footer_left" align="center" colspan="2"><?=$PAGINATION_INFO?></td>
						<td class="table_footer_left" colspan="5"    align="center"><span class="link"><?=$PAGINATION_LINKS?></span></td>
			...
			<?php
				...
				if($num_rows>0){
					...
					for($i=0;$i<$num_rows;$i++){
						...
						$ids 				= mysql_result($result, $i, "processId");
						$processName		= mysql_result($result, $i, "processName");
						$processCode		= mysql_result($result, $i, "processCode");
						$isManual		 	= mysql_result($result, $i, "isManual");
						$isMachine		 	= mysql_result($result, $i, "isMachine");
						$isInspected		= mysql_result($result, $i, "isInspected");
						$isSubcon		 	= mysql_result($result, $i, "isSubcon");
                                                $multiOut		 	= mysql_result($result, $i, "multiOut");
						...
						$link				= "index.php?mod=$mod&type=$type&task=view&sid=$ids";
							...
							echo '	<td align="center" class="table_line2_">
										<input class="ibox" value="'.$ids.'" name="delAnn[]" onclick="checkAllFields(2);" type="checkbox">
									</td>
									<td class="table_line2_left" id="'.$link.'" onClick="viewRecord(event,this.id);">'.stripslashes(html_entity_decode($processName)).'&nbsp;</td>
									<td class="table_line2_left">'.stripslashes(html_entity_decode($processCode)).'&nbsp;</td>
                                                                        <td class="table_line2_left">'.(($isManual==1)?'Yes':'No').'&nbsp;</td>
                                                                        <td class="table_line2_left">'.(($isMachine==1)?'Yes':'No').'&nbsp;</td>
                                                                        <td class="table_line2_left">'.(($isInspected==1)?'Yes':'No').'&nbsp;</td>
                                                                        <td class="table_line2_left">'.(($isSubcon==1)?'Yes':'No').'&nbsp;</td>
                                                                        <td class="table_line2_left_right">'.(($multiOut==1)?'Yes':'No').'&nbsp;</td>';

                                                        echo '</tr>';
					}
				}else{
					echo '<tr valign="middle" width="20px"><td colspan="7" class="table_line2_ table_line2__right" align="center"><p class="errmsg">NO RECORDS FOUND</p></td></tr>';
				}
				?>
				</tbody>
			</table>
